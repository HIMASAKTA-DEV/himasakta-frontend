            ...
            <>
                ...
                <main className="min-h-screen flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <Link href="/news" className="text-primary hover:underline">Kembali ke Berita</Link>
                    </div>
                </main>
                <Footer />
            </>
        ...
        <>
            ...
            <main className="min-h-screen bg-white pt-24 pb-16">
                <div className="container mx-auto px-4 max-w-4xl">
                    <Link href="/news" className="inline-flex items-center gap-2 text-slate-500 hover:text-primary mb-8 transition-colors">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center gap-4 text-sm text-slate-500 mb-4">
                            <span className="flex items-center gap-1 bg-slate-100 px-3 py-1 rounded-full text-slate-700 font-medium">
                                <FaTag size={12} className="text-primary" /> {news.tagline || "Berita"}
                            </span>
                            <span className="flex items-center gap-1">
                                <FaCalendar size={12} /> {format(new Date(news.published_at), "dd MMMM yyyy", { locale: idLocale })}
                            </span>
                        </div>
                    ...
                    {news.hashtags && (
                        <div className="mt-12 pt-8 border-t border-slate-100 flex flex-wrap gap-2">
                            {news.hashtags.split(",").map((tag, idx) => (
                                <span key={idx} className="text-sm text-primary bg-primary/5 px-3 py-1 rounded-full">
                                    #{tag.trim()}
                                </span>
                            ))}
                        </div>
