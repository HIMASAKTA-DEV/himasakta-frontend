...
export const getCabinetInfo = async () => {
    const { data } = await api.get<ApiResponse<CabinetInfo[]>>("/cabinet-info?is_active=true");
...
export const getMonthlyEvents = async () => {
    const { data } = await api.get<ApiResponse<MonthlyEvent[]>>("/monthly-event/this-month");
    return data.data;
};
...
export const getAllNews = async (page = 1, limit = 9, search = "") => {
    const { data } = await api.get<ApiResponse<News[]>>(`/news?page=${page}&limit=${limit}&search=${search}`);
    return data;
};
